...
class GCN(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, nlayers=2, dropout=0.5, lr=0.01, weight_decay=5e-4,#origin:5e-4
    ...
    def fit_with_val(self, features, adj, labels, data, train_iters=200, initialize=True, verbose=False, normalize=True, patience=None, noval=False, finetune=False, **kwargs):
        ...
        if noval:#false inductive=true
            self._train_with_val(labels, data, train_iters, verbose, finetune, adj_val=True)
        else:
            self._train_with_val(labels, data, train_iters, verbose, finetune)

    def _train_with_val(self, labels, data, train_iters, verbose, finetune, adj_val=False):
        if adj_val:
        ...
        for i in range(train_iters):
            ...
            if(finetune==True and adj_val==False):
                output = output[data['idx_train']]
